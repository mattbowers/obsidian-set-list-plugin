...
- `src/views/`:
  ...
  - `SetListEditView.ts` — toolbar (add/remove/reorder via drag-and-drop/enter stage view/drop to raw markdown Source Mode, the escape hatch now that Edit view opens automatically — see `main.ts`). Dragging slides in-between rows out of the way via CSS `transform: translateY(...)` sized to the dragged row's own height. `dragover`/`drop` are handled once at the row-list container, not per-row: per-row handlers were tried first and broke, because shifting a row via `transform` moves it out from under the cursor mid-drag, which retargets/cancels the native drag-and-drop hit-test and made drops silently no-op. The fix snapshots every row's midpoint Y (`dragRowLayout`) once at `dragstart`, before any shuffling starts, and all later insertion-point math (both the shuffle animation and the final `reorder()` call) is computed from `event.clientY` against that frozen snapshot rather than from live element positions or `event.target`.
...
- `src/ui/`:
  - `SongPickerModal.ts` — `FuzzySuggestModal<TFile>` over markdown + PDF vault files. A `Setting`/toggle inserted above the results (via `resultContainerEl.before(...)`, since `Setting`'s constructor always appends) filters to songs tagged with the set list's band tag; on by default, disabled (nothing to filter by) when the set list has no `band` frontmatter. `getItems()` is re-filtered live and re-run by dispatching a synthetic `input` event on `inputEl` — `FuzzySuggestModal` has no public API to force a suggestions refresh otherwise.
